...
CREATE TABLE MY_FAVORIT(
    MY_NUM NUMBER,
    MEM_ID VARCHAR2(10),
    RENT_NUM NUMBER,
    MAEMAE_NUM NUMBER,
    IMDAE_NUM NUMBER,
    CONSTRAINT MY_FAVORIT_PK PRIMARY KEY(MY_NUM),
    CONSTRAINT MY_FAVORIT_FK1 FOREIGN KEY(MEM_ID) 
    REFERENCES APT_MEMBER(MEM_ID),
    CONSTRAINT MY_FAVORIT_FK2 FOREIGN KEY(RENT_NUM) 
    REFERENCES RENT(RENT_NUM),
    CONSTRAINT MY_FAVORIT_FK3 FOREIGN KEY(MAEMAE_NUM) 
    REFERENCES MAEMAE(MAEMAE_NUM)
);

DROP SEQUENCE MY_NUM;
/*즐겨찾기코드 시퀀스 생성*/
CREATE SEQUENCE MY_NUM START WITH 1 INCREMENT BY 1;
SELECT LAST_NUMBER FROM USER_SEQUENCES WHERE SEQUENCE_NAME = 'MY_NUM';

/*인설트가 안됨 왜??
 *안되는 이유 아마도 FK키가 받아야하는 PK가 없어서
 *생성자체가 불가할 것을 생각되네여*/
